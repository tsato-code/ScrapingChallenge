...
def crawling(args):
    ...
    logger.info(f"総レシピ数： {num_recipe}")
    ...
    while True:
        ...
        if next_url is None:
            logger.info(f"総ページ数： {page_count-1}")
        ...
        soup = BeautifulSoup(response.content, "lxml")
        if page_count % 10 == 0:
            logger.info(f"{page_count}ページ取得")
...
if __name__ == "__main__":
    ...
    logger.info(f"クローリング開始: {date_now}")
    main()
    date_now = datetime.datetime.now()
    logger.info(f"クローリング終了: {date_now}")
